...
export default function Overview() {
  ...
  const kpis = [
    ...
    { label: 'High Risk', value: displayStats?.highRiskProjects ?? '—', cls: Number(displayStats?.highRiskProjects) > 0 ? 'text-black' : 'text-black' },
      ...
      <header className="bg-white border-b border-slate-200">
        <div className="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3 min-w-0">
            <Link to="/" className="text-black text-sm font-semibold hover:underline whitespace-nowrap">← Home</Link>
            <h1 className="text-lg font-bold text-black truncate">Parivekshan Dashboard</h1>
          </div>
          <div className="flex items-center gap-3">
            <div className="text-right hidden sm:block">
              <p className="text-xs font-medium text-black truncate max-w-[200px]">{email}</p>
              <p className="text-[10px] font-mono text-black uppercase">{role}</p>
            </div>
            <button
              onClick={clearRole}
              className="text-xs font-mono text-black hover:text-black border border-slate-200 rounded-lg px-2.5 py-1.5 hover:border-red-300 transition whitespace-nowrap"
            >
              Logout
            </button>
          </div>
      ...
      <main className="max-w-6xl mx-auto px-4 py-8">
        {scoped && (
          <div className="mb-6 flex items-center gap-2 bg-navy-500/5 border border-navy-500/20 rounded-xl px-4 py-3">
            ...
            <span className="text-xs text-black">District Magistrate — projects filtered client-side</span>
          </div>
        )}
        ...
        {!loading && error && (
          <div className="bg-white border border-red-200 rounded-xl p-6 max-w-md mx-auto text-center">
            <p className="text-black font-semibold mb-2">Failed to load dashboard</p>
        ...
        {!loading && !error && (
          <>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
              {kpis.map((k) => (
                <div key={k.label} className="bg-white rounded-xl border border-slate-200 p-5">
                  <p className="text-xs font-mono text-black uppercase tracking-wider mb-1">{k.label}</p>
                  <p className={`text-2xl font-bold text-black ${k.cls || ''}`}>{k.value}</p>
                </div>
              ))}
            </div>
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-sm font-mono text-black">{visibleProjects.length} / {projects.length} records</span>
            </div>
            ...
            <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
                  ...
                  {visibleProjects.map((p) => {
                    ...
                    return (
                      <tr key={p.id} className="border-t border-slate-100 hover:bg-slate-50 transition">
                        ...
                        <td className="px-4 py-3 text-black">{p.district || '—'}</td>
              ...
              {visibleProjects.length === 0 && (
                <p className="p-8 text-center text-black">No projects in scope.</p>
              )}
            </div>
          </>
        )}
